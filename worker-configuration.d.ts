/* eslint-disable */
// Generated by Wrangler by running `wrangler types` (hash: b1d61951869130a6f7d2a79fd5efb9dc)
...
type R2Objects = { objects: R2Object[], delimitedPrefixes: string[] } & ({ truncated: true, cursor: string } | { truncated: false });
...
type AiAutomaticSpeechRecognitionOutput = { text?: string, words?: { word: string, start: number, end: number }[], vtt?: string };
...
type AiTextGenerationOutput = { response?: string, tool_calls?: { name: string, arguments: unknown }[] };
...
type ConversionResponse = { name: string, mimeType: string, format: 'markdown', tokens: number, data: string };
...
declare abstract class Ai<AiModelList extends AiModelListType = AiModels> {
  ...
  run<Name extends keyof AiModelList, Options extends AiOptions, InputOptions extends AiModelList[Name]['inputs']>(
    ...
    Options extends { returnRawResponse: true } ? Response :
      InputOptions extends { stream: true } ? ReadableStream :
...
type GatewayRetries = { maxAttempts?: 1 | 2 | 3 | 4 | 5, retryDelayMs?: number, backoff?: 'constant' | 'linear' | 'exponential' };
type GatewayOptions = {
  id: string,
  cacheKey?: string,
  cacheTtl?: number,
...
type ComparisonFilter = { key: string, type: 'eq' | 'ne' | 'gt' | 'gte' | 'lt' | 'lte', value: string | number | boolean };
...
declare module 'cloudflare:email' {
  let _EmailMessage: { prototype: EmailMessage, new (from: string, to: string, raw: ReadableStream | string): EmailMessage };
...
type ImageInfoResponse = { format: 'image/svg+xml' } | { format: string, fileSize: number, width: number, height: number };
type ImageTransform = {
  width?: number,
  height?: number,
  background?: string,
...
type ImageDrawOptions = { opacity?: number, repeat?: boolean | string, top?: number, left?: number, bottom?: number, right?: number };
...
declare namespace Rpc {
  ...
  type Stubify<T> = T extends Stubable ? Stub<T> :
    T extends Map<infer K, infer V> ? Map<Stubify<K>, Stubify<V>> : T extends Set<infer V> ? Set<Stubify<V>> :
  ...
  type Unstubify<T> = T extends StubBase<infer V> ? V :
    T extends Map<infer K, infer V> ? Map<Unstubify<K>, Unstubify<V>> : T extends Set<infer V> ? Set<Unstubify<V>> :
...
declare module 'cloudflare:node' {
  ...
  export function httpServerHandler(options: { port: number }, handlers?: Omit<DefaultHandler, 'fetch'>): DefaultHandler;
}
